...
from models.renderer_bsdf import BSDFRenderer
...
BSDF_PARAMS = ["base_color", "subsurface", "metallic", "specular", "specular_tint", "roughness", "anisotropic", "sheen", "sheen_tint", "clearcoat", "clearcoat_gloss"]
...
def get_materials(color_network_dict, points, normals, features, is_metal=args.is_metal):
    bsdf_params = {}
    for p in BSDF_PARAMS:
        val = color_network_dict[p + "_network"](points, normals, None, features)
        bsdf_params[p] = val
    return bsdf_params
...
def render_fn(interior_mask, color_network_dict, ray_o, ray_d, points, normals, features):
    ...
    if interior_mask.any():
        ...
        bsdf_params = get_materials(
            color_network_dict, points, normals, features
        )
        results = bsdf_renderer(
...
for p in BSDF_PARAMS:
    color_network_dict[p + "_network"] = RenderingNetwork(
        d_in=9,
        d_out=3,
        d_feature=256,
        d_hidden=256,
...
for p in BSDF_PARAMS:
    color_optimizer_dict[p + "_network"] = torch.optim.Adam(
        color_network_dict[p + "_network"].parameters(), lr=1e-4
    )
    

###### loss specifications
# ggx_renderer = GGXColocatedRenderer(use_cuda=True)
bsdf_renderer = BSDFRenderer(use_cuda=True)
...
if os.path.isfile(args.neus_ckpt_fpath):
    ...
    try:
        ...
        color_network_dict["base_color_network"].load_state_dict(ckpt["color_network_fine"])
    except:
        traceback.print_exc()
